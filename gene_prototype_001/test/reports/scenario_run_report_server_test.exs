defmodule GenePrototype0001.Reports.ScenarioRunReportServerTest do
  ...
  test "submits and retrieves reports" do
    ...
    agents = [
      %{
        ...
        numina: ["GenePrototype0001.Numina.BasicMotionNumen"]
      },
      %{
        id: "agent_2",
        actuator_commands: 5,
    ...
    assert agent1.numina == ["GenePrototype0001.Numina.BasicMotionNumen"]

    # Verify the second agent
    agent2 = Enum.find(report.agents, fn a -> a.id == "agent_2" end)
    assert agent2.actuator_commands == 5
    assert agent2.numina == ["GenePrototype0001.Numina.BasicMotionNumen"]
  end

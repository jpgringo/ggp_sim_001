defmodule GeneticsEngine.Bandit.SelectiveLogger do
  @behaviour Plug
...
defmodule GeneticsEngine.Bandit.Router do
  use Plug.Router

  # Don't log /api/status requests to avoid log flooding from polling
  plug GeneticsEngine.Bandit.SelectiveLogger
  ...
  get "/ws/scenario" do
    case conn.method do
      "GET" ->
        conn
        |> WebSockAdapter.upgrade(GeneticsEngine.SimulationSocket, [], timeout: 60_000)
        |> halt()
      _ ->

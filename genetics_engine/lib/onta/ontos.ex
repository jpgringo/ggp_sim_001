defmodule GeneticsEngine.Onta.Ontos do
  use GenServer
  require Logger
  require DirectDebug
...
#    name = via_tuple(agent_id)
  ...
  def init({unique_id, opts}) do
    ...
    {:ok, numen_sup} = GeneticsEngine.Numina.NumenSupervisor.start_link(unique_id)

    # Start initial Numina
    numen_pids = for numen_module <- numina do
      DirectDebug.info("Starting Numen: #{inspect(numen_module)}")
      {:ok, pid} = GeneticsEngine.Numina.NumenSupervisor.start_numen(numen_sup, numen_module, unique_id)
      pid
    end
  ...
  def handle_call({:add_numen, numen_module}, _from, state) do
    case GeneticsEngine.Numina.NumenSupervisor.start_numen(state.numen_supervisor, numen_module, state.raw_id) do
      {:ok, pid} ->
  ...
  def handle_call({:remove_numen, numen_pid}, _from, state) do
    case GeneticsEngine.Numina.NumenSupervisor.terminate_numen(state.numen_supervisor, numen_pid) do
      :ok ->
  ...
  defp process_incoming_sensor_set(sensor_data_set, state) do
    ...
    Enum.each(state.numen_pids, fn pid ->
      GeneticsEngine.Numina.Numen.process_sensor_data_set(pid, sensor_data_set)
    end)

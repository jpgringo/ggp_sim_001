defmodule DirectDebug do
  ...
  def extra(text, alsoLog? \\ false) do
    if Application.get_env(:genetics_engine, :direct_log_level, :none) in [:all] do
  ...
  def section(text, alsoLog? \\ false) do
    if Application.get_env(:genetics_engine, :direct_log_level, :none) in [:info, :all] do
  ...
  def warning(text, alsoLog? \\ false) do
    if Application.get_env(:genetics_engine, :direct_log_level, :none) in [:warn, :info, :all] do
  ...
  def error(text, alsoLog? \\ false) do
    if Application.get_env(:genetics_engine, :direct_log_level, :none) in [:error, :warn, :info, :all] do
